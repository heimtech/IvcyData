...
/**
 * Class FormColumnStart
 *
 * Form field "column" (start).
 */
class FormColumnStart extends \Widget
{
     ...
     * @var string
     */
    protected $strTemplate = 'form_t4c_column_start';
    ...
    public function __construct($arrAttributes=null)
    {
        ...
        //$intCurrent = $GLOBALS['T4C_COLUMN_WRAPPER']['FORM']['current'];
        //$intCurrent = isset($GLOBALS['T4C_COLUMN_WRAPPER']['FORM']['columns'][$intCurrent]) ? $intCurrent : null;

        //$this->column = $GLOBALS['T4C_COLUMN_WRAPPER']['FORM']['columns'][$intCurrent];
        //$this->column_tablet = $GLOBALS['T4C_COLUMN_WRAPPER']['FORM']['columns_tablet'][$intCurrent];
        //$this->column_phone = $GLOBALS['T4C_COLUMN_WRAPPER']['FORM']['columns_phone'][$intCurrent];
		...
		if ($columnStyle){
			...
			if ($textColor){
				$GLOBALS['T4C_CSS'][] = '#t4c_column_' . $this->id . ' h1,#t4c_column_' . $this->id . ' h2,#t4c_column_' . $this->id . ' h3,#t4c_column_' . $this->id . ' h4,#t4c_column_' . $this->id . ' h5,#t4c_column_' . $this->id . ' h6,#t4c_column_' . $this->id . ' strong,#t4c_column_' . $this->id . ' label{color:' . $textColor . ';}';
			}
		}
        ...
        if (TL_MODE == 'BE')
        {
			...
			$this->title = '<span class="invisible t4c_column_start"></span><span style="opacity: 0.6;">Large width: ' . $this->t4c_column_large_width . ' | Medium width: ' . $this->t4c_column_medium_width . ' |</span> Small width: ' . $this->t4c_column_width ;
			if ($this->t4c_column_xsmall_width){
				$this->title .=  '<span style="opacity: 0.6;">| Extra small width: ' . $this->t4c_column_xsmall_width . '</span>';
			}
        }
    }
